...
export class QuickBooksOAuthService {
  ...
  private handleQuickBooksError(error: any): never {
    if (error.response && error.response.data && error.response.data.Fault) {
      const fault = error.response.data.Fault;
      const errors = fault.Error;

      if (errors && errors.length > 0) {
        const qbError = errors[0];
        const code = qbError.code;
        const message = qbError.Message;
        const detail = qbError.Detail;
        const type = fault.type;

        const exceptionMessage = `${message} - ${detail}`;
        ...
        switch (code) {
          ...
          default:
            // For other error codes, throw a general exception
            throw new InternalServerErrorException(
              `QuickBooks API error (Code ${code}): ${exceptionMessage}`,
            );
        }
      } else {
        throw new InternalServerErrorException('Unknown error from QuickBooks API.');
      }
    } else if (error.response && error.response.status === 401) {
      throw new UnauthorizedException('Unauthorized access to QuickBooks API.');
    } else {
      // Log the error for debugging
      console.error('Error in QuickBooks API call:', error);

      // Throw a generic error
      throw new InternalServerErrorException(
        'An error occurred while communicating with QuickBooks API.',
      );
    }
  }
  ...
  async refreshTokenIfNeeded(clientId: string): Promise<void> {
    ...
    if (currentTime >= tokenExpirationTime - 5 * 60 * 1000) {
      try {
        ...
        this.handleQuickBooksError(error);
      }
    }
  }
      ...
      const response = await this.oauthClient.makeApiCall({
        url: `${
          this.oauthClient.environment === 'sandbox'
            ? 'https://sandbox-quickbooks.api.intuit.com'
            : 'https://quickbooks.api.intuit.com'
        }/v3/company/${client.quickbooksRealmId}/customer`,
        method: 'GET',
        headers: {
          Accept: 'application/json',
        },
    ...
    try {
      const url = `${
        this.oauthClient.environment === 'sandbox'
          ? 'https://sandbox-quickbooks.api.intuit.com'
          : 'https://quickbooks.api.intuit.com'
      }/v3/company/${realmId}/item/service`;
  ...
  // Method to create a service in QuickBooks
  async createService(realmId: string, accessToken: string, serviceData: any) {
    try {
      const quickBooksBaseUrl =
        this.oauthClient.environment === 'sandbox'
