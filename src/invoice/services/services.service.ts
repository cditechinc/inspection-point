...
export class ServicesService {
  ...
  async createServiceFee(
    ...
    try {
      ...
      console.log(
        'Creating service in QuickBooks with data:',
        quickBooksServiceData,
      );

      const createdQuickBooksService =
        await this.quickBooksService.createService(
      ...
      for (const fee of serviceFees) {
        ...
        if (existingServiceFee) {
          ...
          await this.serviceFeeRepository.update(
            existingServiceFee.id,
            serviceFeeData,
          );
        } else {
          // Create a new service fee
          const newServiceFee =
            this.serviceFeeRepository.create(serviceFeeData);
