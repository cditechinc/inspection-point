...
export class AssetsService {
  ...
  async update(
    ...
    //   if (!assetType) {
    //     throw new NotFoundException(
    //       `AssetType #${updateAssetDto.assetTypeId} not found`,
    //     );
    //   }
   ...
   private validateProperties(
    ...
    switch (assetTypeName) {
      ...
      case 'Treatment Plant Digester':
        this.validateTreatmentPlantDigesterProperties(properties as TreatmentPlantDigesterProperties);
        break;
      case 'Lint Trap':
  ...
  private validateLiftStationProperties(properties: LiftStationProperties) {
    ...
    if (properties.pipeDia !== undefined) {
      if (typeof properties.pipeDia !== 'string') {
        throw new BadRequestException('Pipe Diameter (pipeDia) must be a string.');
      }
    }
    ...
    if (properties.material !== undefined) {
      const validMaterials = ['Plastic', 'Fiberglass', 'Concrete', 'Other', 'Unknown'];
      if (!validMaterials.includes(properties.material)) {
        throw new BadRequestException(`Material must be one of: ${validMaterials.join(', ')}.`);
      }
    }
    ...
    if (properties.duty !== undefined) {
      ...
      if (!validDuties.includes(properties.duty)) {
        throw new BadRequestException(`Duty must be one of: ${validDuties.join(', ')}.`);
      }
    }
    ...
    if (properties.inspectionInterval !== undefined) {
      const validIntervals = ['Weekly', 'Monthly', 'Bi-Monthly', 'Quarterly', 'Yearly', 'On-Demand', 'Not Serviced'];
      if (!validIntervals.includes(properties.inspectionInterval)) {
        throw new BadRequestException(`Inspection Interval must be one of: ${validIntervals.join(', ')}.`);
      }
    }
  ...
  private validateGreaseTrapProperties(properties: GreaseTrapProperties) {
    ...
    if (!properties.serviceInterval) {
      throw new BadRequestException('Service Interval is required for Grease Trap assets.');
    } else {
      const validIntervals = ['Weekly', 'Monthly', 'Bi-Monthly', 'Quarterly', 'Yearly', 'On-Demand', 'Not Serviced'];
      if (!validIntervals.includes(properties.serviceInterval)) {
        throw new BadRequestException(`Service Interval must be one of: ${validIntervals.join(', ')}.`);
      }
    }
  
    // Validate gallons
    if (properties.gallons === undefined || properties.gallons === null) {
      throw new BadRequestException('Gallons is required for Grease Trap assets.');
    ...
    if (!properties.material) {
      throw new BadRequestException('Material is required for Grease Trap assets.');
    } else {
      const validMaterials = ['Plastic', 'Fiberglass', 'Concrete', 'Other', 'Unknown'];
      if (!validMaterials.includes(properties.material)) {
        throw new BadRequestException(`Material must be one of: ${validMaterials.join(', ')}.`);
    ...
    if (!properties.latitude) {
      throw new BadRequestException('Latitude is required for Grease Trap assets.');
    ...
    if (!properties.longitude) {
      throw new BadRequestException('Longitude is required for Grease Trap assets.');
    ...
    if (!properties.qrCode) {
      throw new BadRequestException('QR Code is required for Grease Trap assets.');
    ...
    // Validate nfcId
    if (!properties.nfcId) {
      throw new BadRequestException('NFC ID is required for Grease Trap assets.');
    ...
    if (properties.requireDisposalTicket === undefined || properties.requireDisposalTicket === null) {
      throw new BadRequestException('Require Disposal Ticket is required for Grease Trap assets.');
    } else if (typeof properties.requireDisposalTicket !== 'boolean') {
      throw new BadRequestException('Require Disposal Ticket must be a boolean.');
    }
  
    // Validate eveningService
    if (properties.eveningService === undefined || properties.eveningService === null) {
      throw new BadRequestException('Evening Service is required for Grease Trap assets.');
    ...
    if (properties.multipleOnSiteTraps === undefined || properties.multipleOnSiteTraps === null) {
      throw new BadRequestException('Multiple On-Site Traps is required for Grease Trap assets.');
    } else if (typeof properties.multipleOnSiteTraps !== 'boolean') {
      throw new BadRequestException('Multiple On-Site Traps must be a boolean.');
    }
  ...
  private validateTreatmentPlantDigesterProperties(
    ...
    // Validate serviceInterval
    if (!properties.serviceInterval) {
      throw new BadRequestException('Service Interval is required for Treatment Plant Digester assets.');
    } else {
      const validIntervals = ['Weekly', 'Monthly', 'Bi-Monthly', 'Quarterly', 'Yearly', 'On-Demand', 'Not Serviced'];
      if (!validIntervals.includes(properties.serviceInterval)) {
        throw new BadRequestException(`Service Interval must be one of: ${validIntervals.join(', ')}.`);
    ...
    if (!properties.gallons) {
      throw new BadRequestException('Gallons is required for Treatment Plant Digester assets.');
    ...
    // Validate material
    if (!properties.material) {
      throw new BadRequestException('Material is required for Treatment Plant Digester assets.');
    } else {
      const validMaterials = ['Plastic', 'Fiberglass', 'Concrete', 'Other', 'Unknown'];
      if (!validMaterials.includes(properties.material)) {
        throw new BadRequestException(`Material must be one of: ${validMaterials.join(', ')}.`);
    ...
    if (properties.suctionRequired === undefined || properties.suctionRequired === null) {
    ...
    if (!properties.digesterDimensions) {
      ...
      if (!dimensionPattern.test(properties.digesterDimensions)) {
        throw new BadRequestException('Digester Dimensions must be in the format "000 x 000 x 000".');
      }
    }
    ...
    if (properties.primaryTreatmentPlantAssetId !== undefined) {
      if (typeof properties.primaryTreatmentPlantAssetId !== 'string') {
        throw new BadRequestException('Primary Treatment Plant Asset ID must be a string.');
      }
      // Optionally, validate UUID format
      const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
      if (!uuidPattern.test(properties.primaryTreatmentPlantAssetId)) {
        throw new BadRequestException('Primary Treatment Plant Asset ID must be a valid UUID.');
      }
    }
    ...
    if (!properties.condition) {
      ...
      const validConditions = ['Good', 'Fair', 'Rough', 'Bad', 'Failing', 'Other'];
  ...
  private validateLintTrapProperties(properties: LintTrapProperties) {
    // Validate serviceInterval
    if (!properties.serviceInterval) {
      throw new BadRequestException('Service Interval is required for Lint Trap assets.');
    } else {
      const validIntervals = ['Weekly', 'Monthly', 'Bi-Monthly', 'Quarterly', 'Yearly', 'On-Demand', 'Not Serviced'];
      if (!validIntervals.includes(properties.serviceInterval)) {
        throw new BadRequestException(`Service Interval must be one of: ${validIntervals.join(', ')}.`);
    ...
    // Validate gallons
    if (!properties.gallons) {
      throw new BadRequestException('Gallons is required for Lint Trap assets.');
    } else if (typeof properties.gallons !== 'string') {
      throw new BadRequestException('Gallons must be a string.');
    }
    ...
    // Validate material
    if (!properties.material) {
      throw new BadRequestException('Material is required for Lint Trap assets.');
    } else {
      const validMaterials = ['Plastic', 'Fiberglass', 'Concrete', 'Other', 'Unknown'];
      if (!validMaterials.includes(properties.material)) {
        throw new BadRequestException(`Material must be one of: ${validMaterials.join(', ')}.`);
    ...
    // Validate latitude
    if (!properties.latitude) {
      throw new BadRequestException('Latitude is required for Lint Trap assets.');
    } else if (typeof properties.latitude !== 'string') {
      throw new BadRequestException('Latitude must be a string.');
    }
    ...
    // Validate longitude
    if (!properties.longitude) {
      throw new BadRequestException('Longitude is required for Lint Trap assets.');
    } else if (typeof properties.longitude !== 'string') {
      throw new BadRequestException('Longitude must be a string.');
    }
    ...
    // Validate qrCode
    if (!properties.qrCode) {
      throw new BadRequestException('QR Code is required for Lint Trap assets.');
    } else if (typeof properties.qrCode !== 'string') {
      throw new BadRequestException('QR Code must be a string.');
    }
    ...
    // Validate requireDisposalTicket
    if (properties.requireDisposalTicket === undefined || properties.requireDisposalTicket === null) {
      throw new BadRequestException('Require Disposal Ticket is required for Lint Trap assets.');
    } else if (typeof properties.requireDisposalTicket !== 'boolean') {
      throw new BadRequestException('Require Disposal Ticket must be a boolean.');
    }
    ...
    // Validate eveningService
    if (properties.eveningService === undefined || properties.eveningService === null) {
      throw new BadRequestException('Evening Service is required for Lint Trap assets.');
    } else if (typeof properties.eveningService !== 'boolean') {
      throw new BadRequestException('Evening Service must be a boolean.');
    }
    ...
    // Validate multipleOnSiteTraps
    if (properties.multipleOnSiteTraps === undefined || properties.multipleOnSiteTraps === null) {
      throw new BadRequestException('Multiple On-Site Traps is required for Lint Trap assets.');
    } else if (typeof properties.multipleOnSiteTraps !== 'boolean') {
      throw new BadRequestException('Multiple On-Site Traps must be a boolean.');
    }
